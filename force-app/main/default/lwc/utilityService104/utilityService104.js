...
export default class utilityService104 extends LightningElement {
    @api recordId;
    @api config = {};
    ...
    loadConfig() {
        const saved = sessionStorage.getItem('utilityService104_config');
        if (saved) {
            try {
                this.config = { ...this.config, ...JSON.parse(saved) };
            } catch (e) {
                console.warn('utilityService104: Failed to parse saved config');
            }
        }
    }
