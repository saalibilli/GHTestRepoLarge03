...
export default class utilityService99 extends LightningElement {
    @api recordId;
    @api config = {};
    ...
    loadConfig() {
        const saved = sessionStorage.getItem('utilityService99_config');
        if (saved) {
            try {
                this.config = { ...this.config, ...JSON.parse(saved) };
            } catch (e) {
                console.warn('utilityService99: Failed to parse saved config');
            }
        }
    }
