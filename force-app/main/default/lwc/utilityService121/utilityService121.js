...
export default class utilityService121 extends LightningElement {
    @api recordId;
    @api config = {};
    ...
    loadConfig() {
        const saved = sessionStorage.getItem('utilityService121_config');
        if (saved) {
            try {
                this.config = { ...this.config, ...JSON.parse(saved) };
            } catch (e) {
                console.warn('utilityService121: Failed to parse saved config');
            }
        }
    }
