...
export default class utilityService118 extends LightningElement {
    @api recordId;
    @api config = {};
    ...
    loadConfig() {
        const saved = sessionStorage.getItem('utilityService118_config');
        if (saved) {
            try {
                this.config = { ...this.config, ...JSON.parse(saved) };
            } catch (e) {
                console.warn('utilityService118: Failed to parse saved config');
            }
        }
    }
