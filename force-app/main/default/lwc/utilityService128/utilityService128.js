...
export default class utilityService128 extends LightningElement {
    @api recordId;
    @api config = {};
    ...
    loadConfig() {
        const saved = sessionStorage.getItem('utilityService128_config');
        if (saved) {
            try {
                this.config = { ...this.config, ...JSON.parse(saved) };
            } catch (e) {
                console.warn('utilityService128: Failed to parse saved config');
            }
        }
    }
